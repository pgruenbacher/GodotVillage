...
Mesh Tile::generate() {
	...
	for (unsigned int i = 0; i < nbDiv + 1; ++i) {
		Vector3F current = center0 + bottomRadius * cosf(float(i) * M_PI / nbDiv) * normalize(quad.p0() - center0) + bottomRadius * sinf(float(i) * M_PI / nbDiv) * quad.normal();
		points.pushLast(current);
	}
	for (unsigned int i = 0; i < nbDiv + 1; ++i) {
		Vector3F current = center1 + topRadius * cosf(float(i) * M_PI / nbDiv) * normalize(quad.p1() - center1) + topRadius * sinf(float(i) * M_PI / nbDiv) * quad.normal();
		points.pushLast(current);
	}
	...
	for (unsigned int i = 0; i < nbDiv + 1; ++i) {
		Vector3F current = center0 + bottomRadius2 * cosf(float(i) * M_PI / nbDiv) * normalize(quad.p0() - center0) + bottomRadius2 * sinf(float(i) * M_PI / nbDiv) * quad.normal();
		points2.pushLast(current);
	}
	for (unsigned int i = 0; i < nbDiv + 1; ++i) {
		Vector3F current = center1 + topRadius2 * cosf(float(i) * M_PI / nbDiv) * normalize(quad.p1() - center1) + topRadius2 * sinf(float(i) * M_PI / nbDiv) * quad.normal();
		points2.pushLast(current);
	}
