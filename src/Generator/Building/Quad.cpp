...
float getDiffHeight(Vector3F p0, Vector3F p1) {
	...
	pos1.y = -p1.z;
	float h0, h1;
	h0 = VillageGenerator::_instance->_heightmapLayout->getValueAt(pos0) * VillageGenerator::_instance->_terrain->getMaxElevation();
	h1 = VillageGenerator::_instance->_heightmapLayout->getValueAt(pos1) * VillageGenerator::_instance->_terrain->getMaxElevation();
	return h1 - h0;
}
...
Facade::ConstraintType analysis(FacadeSubdiv facade) {
	...
	for (unsigned int i = 0; i < int(length); ++i) {
		Vector3F pos = p0 * float(int(length) - 1 - i) / float(int(length) - 1) + p1 * float(i) / float(int(length) - 1);
...
bool canContain(Quad quad, Quad to) {
	...
	if (quadWidthT > toWidthT && quadWidthT > toWidthB && quadWidthB > toWidthT && quadWidthB > toWidthB && quadHeightR > toHeightR && quadHeightR > toHeightL && quadHeightL > toHeightR && quadHeightL > toHeightL) {
		return true;
	}
	return false;
}
...
bool validWindowPosition(Quad quad, Quad to) {
	...
	if (to.p0().y > minHeight && to.p1().y > minHeight && to.p2().y < maxHeight && to.p3().y < maxHeight) {
		float dot1 = dotProduct(to.p0() - quad.p0(), normalize(quad.p01()));
		float dot2 = dotProduct(to.p1() - quad.p0(), normalize(quad.p01()));
		if (dot1 > 0.1 && dot2 < norm(quad.p01()) - 0.1) {
...
Quad placeOnQuad(Quad quad, Quad to) {
	...
	result.p0() = quad.p0() + to.p0().x * p01 + to.p0().y * p03;

	result.p1() = quad.p0() + to.p1().x * p01 + to.p1().y * p03;

	result.p2() = quad.p0() + to.p2().x * p01 + to.p2().y * p03;

	result.p3() = quad.p0() + to.p3().x * p01 + to.p3().y * p03;
