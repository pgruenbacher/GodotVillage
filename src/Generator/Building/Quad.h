...
struct Quad {
	...
	Table<Quad> subdiviseX(unsigned int nb) {
		...
		for (unsigned int i = 0; i < nb; ++i) {
			pp0 = float(nb - i) / float(nb) * _points[0] + float(i) / float(nb) * _points[1];

			pp1 = float(nb - i - 1) / float(nb) * _points[0] + float(i + 1) / float(nb) * _points[1];

			pp2 = float(nb - i - 1) / float(nb) * _points[3] + float(i + 1) / float(nb) * _points[2];

			pp3 = float(nb - i) / float(nb) * _points[3] + float(i) / float(nb) * _points[2];
	...
	Table<Quad> cylinder(unsigned int nbDiv) {
		...
		for (unsigned int i = 0; i < nbDiv + 1; ++i) {
			Vector3F current = center0 + bottomRadius * cosf(float(i) * M_PI / nbDiv) * normalize(p0() - center0) + bottomRadius * sinf(float(i) * M_PI / nbDiv) * normal();
			points.pushLast(current);
		}
		for (unsigned int i = 0; i < nbDiv + 1; ++i) {
			Vector3F current = center1 + topRadius * cosf(float(i) * M_PI / nbDiv) * normalize(p1() - center1) + topRadius * sinf(float(i) * M_PI / nbDiv) * normal();
			points.pushLast(current);
		}
