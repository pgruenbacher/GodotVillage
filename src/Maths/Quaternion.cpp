...
Quaternion::Quaternion(float angleInRad,
		Vector3F vvector)
		: vector(vvector), angle(angleInRad) {
	_correctAngle();
}
...
Quaternion::Quaternion(float aangleInRad,
		...
		float y,
		float z)
		: vector(Vector3F(x, y, z)), angle(aangleInRad) {
	_correctAngle();
}
...
Quaternion::Quaternion(const Quaternion &quat)
		: vector(quat.vector), angle(quat.angle) {
}
...
Quaternion operator*(const Quaternion &quat1, const Quaternion &quat2) {
	...
	result.angle = quat1.angle * quat2.angle - dotProduct(quat1.vector, quat2.vector);

	result.vector = quat1.angle * quat2.vector + quat2.angle * quat1.vector + crossProduct(quat1.vector, quat2.vector);
...
float norm(const Quaternion &quat) {
	return sqrt(float(quat.angle * quat.angle + quat.vector.x * quat.vector.x + quat.vector.y * quat.vector.y + quat.vector.z * quat.vector.z));
}
