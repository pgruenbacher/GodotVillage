...
public :
    ...
    {
        if ( (_zero.x + _node->getPosition().x <= point.x)
                && (_zero.x + _node->getPosition().x + _width >=  point.x)
                && (_zero.y + _node->getPosition().y <= point.y)
                && (_zero.y + _node->getPosition().y + _height >=  point.y) )
        {
            return true;
        }
