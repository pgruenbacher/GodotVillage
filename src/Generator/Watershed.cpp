...
{	
	if ( (fabs(dotProduct(Vector2F(source->pos - source->origin), source->normal)) > source->max)
	     || (fabs(dotProduct(Vector2F(source->pos - source->origin), source->tangent)) > source->max))
		return;
...
void Watershed::getQuadIn(unsigned int id)
{
	...
	while (size > 3 && ok)
	{
		...
		if ( (distanceToAPolygon(HousePos + size * (n + t), polygon) < 0.0)
		     && (distanceToAPolygon(HousePos + size * (n -1.f* t), polygon) < 0.0)
		     && (distanceToAPolygon(HousePos + size * (-1.f *n + t), polygon) < 0.0)
		     && (distanceToAPolygon(HousePos + size * (-1.f*n - 1.f*t), polygon) < 0.0))
		{
			ok = false;
		}
...
void Watershed::perform()
{
	...
	*/
	...
	for (unsigned int i = 1; i <= _sources.getSize() ; ++i)
	{
		...
		else if (_sources[i-1].building.value()._type == Building::CHURCH
			|| _sources[i-1].building.value()._type == Building::VILLA)
		{
			getQuadIn(i);
			getPolygon(i);
			//convexify(i);
...
{	
	...
	for (unsigned int w = 0; w < _image->getWidth(); ++w)
	{
		for (unsigned int h = 0; h < _image->getHeight(); ++h)
		{
			...
			{
				if ( (fabs(_image->getAt(w+1,h).bf() - (idvalue)) < inf )
				     && (fabs(_image->getAt(w-1,h).bf() - (idvalue)) < inf )
				     && (fabs(_image->getAt(w,h+1).bf() - (idvalue)) < inf )
				     && (fabs(_image->getAt(w,h-1).bf() - (idvalue)) < inf ))
				{}
				else
				{
					...
					if (_sources[id-1].building.value()._type != Building::HOUSE)
					{
						...
						if (fabs(_image->getAt(w,h+1).bf() - (idvalue)) < inf 
						    &&  (fabs(_image->getAt(w,h-1).bf() - (idvalue)) > inf ) )
						{
							offy = -0.5;
							neigh++;
