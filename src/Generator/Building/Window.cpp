...
#include "Window.h"
        ...
        Mesh Window::generate()
	{
		...
		float terrainHeight0 = VillageGenerator::_instance->_heightmapLayout->getValueAt(pos0)
			* VillageGenerator::_instance->_terrain->getMaxElevation();

		Vector2I pos1(_quad.p1().x, -_quad.p1().z);
		float terrainHeight1 = VillageGenerator::_instance->_heightmapLayout->getValueAt(pos1)
			* VillageGenerator::_instance->_terrain->getMaxElevation();
		...
		case SQUARE :
		{
			...
			// SHUTTERS SUPPORT
			Vector3F ss0 = windowPart.p0() 
				- 0.05f * normalize(windowPart.p01()) 
				+ 0.2f  * normalize(windowPart.p03())
				+ 0.05f * windowPart.normal();

			Vector3F ss1 = windowPart.p1() 
				+ 0.05f * normalize(windowPart.p01()) 
				+ 0.2f  * normalize(windowPart.p03())
				+ 0.05f * windowPart.normal();

			Vector3F ss2 = windowPart.p2() 
				+ 0.05f * normalize(windowPart.p32())
				- 0.2f  * normalize(windowPart.p03())
				+ 0.05f * windowPart.normal();

			Vector3F ss3 = windowPart.p3() 
				- 0.05f * normalize(windowPart.p32()) 
				- 0.2f  * normalize(windowPart.p03())
				+ 0.05f * windowPart.normal();
