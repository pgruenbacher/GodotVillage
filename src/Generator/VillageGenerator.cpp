...
: _terrain(terrain)
, _width(terrain->getWidth())
, _height(terrain->getHeight())
, _landscapeGraph(NULL)
, _scale(4)
, _neighbourhood(THIRTY_TWO)
, _bezier(false)
{
	...
	for (unsigned int i = 0; i < _height; i+=2)
	{
		for (unsigned int j = 0; j < _width; j+=2)
		{
			if (_vegetationLayout->getImage()->getAt(j,i).r() > 0.96  && _waterLayout->getImage()->getAt(j,i).r() <= 0.5)
			{
				...
				if (random > 0.98)
				{
					float height = _terrain->getHeightMap()->getAt(j,i).rf()
						* _terrain->getMaxElevation();
...
float VillageGenerator::militaryInterest   (Building& building)
{
        ...
        {
                it.next();
                if ((it.value()._type == Building::TOWER_DEFENSE)
                    || (it.value()._type == Building::CASTLE))
                {
...
float VillageGenerator::cultInterest       (Building& building)
{
        ...
        {
                it.next();
                if ((it.value()._type == Building::CHURCH)
                    || (it.value()._type == Building::STATUE))
                {
                        dist = MIN(norm(position - it.value().getNode().getPosition())-it.value()._spaceMin-building._spaceMin, dist);
                        ++nb;
                }
...
float VillageGenerator::sociabilityInterest(Building& building)
{
        ...
        {
                it.next();
                if ((it.value()._type == Building::HOUSE)
                    || (it.value()._type == Building::VILLA)
                    || (it.value()._type == Building::TRADE)
                    || (it.value()._type == Building::WATER_MILL)
                    || (it.value()._type == Building::WIND_MILL))
                {
                        dist = MIN(norm(position - it.value().getNode().getPosition())-it.value()._spaceMin-building._spaceMin, dist);
                        ++nb;
                }
...
void VillageGenerator::positionCluster(Building& building, int nb)
{
	...
	while (i < nb)
	{
		...
		while (nbTests < nbTestMax)
		{
			...
			if (validPosition(current.getNode().getPosition())
			    && feasabilityTest(current))
			{
				if (_village.getRoads().getSize() == 0)
				{
	...
	int nb1 = 0;
		...
		while (itBIS.hasNext())
		{
			...
			end2   = itBIS.value()._path.getLast();
		        ...
		        if (norm (start1 - start2) < maxDist
				&& norm (start1 - start2) > 0)
		        {
